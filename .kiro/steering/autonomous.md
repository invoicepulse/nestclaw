...
Behavior Rules
Do not pause to ask questions. Make all decisions yourself based on the steering files.
Do not ask for confirmation on library choices, file naming, or architecture decisions.
When blocked by missing credentials, add a clearly labeled `// MISSING: ENV_VAR_NAME` comment and continue implementing the rest of the function.
Follow tasks.md strictly — complete tasks in order, do not skip ahead.
After completing each task group, run `pnpm check` and `pnpm test` before continuing.
Fix any TypeScript errors immediately — do not leave type errors.
Write real implementations: Every service function (caddy, cloudflare, docker, resend) must have the actual HTTP calls, not empty shells.
Build the Dockerfiles completely — they must be runnable with `docker build`.
The webhook handler is the most critical piece — implement it fully with signature verification, error handling, and all side effects.
